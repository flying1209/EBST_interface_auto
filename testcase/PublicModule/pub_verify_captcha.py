...
#---------------EBST  公共部分 ----------------------
class pub_verify_captcha(unittest.TestCase):

    def setUp(self):

        # 文档获取验证码接口：http://beta-store.elfbar.com/captcha-validation
        # 实际地址：https://beta-store.elfbar.com/coreapi/captcha-validation

        self.Url = Url().test_url()+"/coreapi/captcha-validation"
        print(self.Url)
    ...
    def testcase_001(self):
        ...
        # 第一次访问data为true，第二次为false验证码已经失效
        param={"captcha":"tnnp",
               "captcha_key":"$2y$10$nMKrGphU7O0Wodc8vkrp2O2KFP58LD8wsguy6gTckeMxYHJoCm8XS"}
