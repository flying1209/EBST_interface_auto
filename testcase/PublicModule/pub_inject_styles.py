...
#---------------EBST  公共部分 ----------------------
class pub_inject_styles(unittest.TestCase):

    def setUp(self):

        # 文档获取注入样式：http://beta-store.elfbar.com/inject-styles
        # 实际地址：https://beta-store.elfbar.com/coreapi/inject-styles

        self.Url = Url().test_url()+"/coreapi/inject-styles"
        print(self.Url)
