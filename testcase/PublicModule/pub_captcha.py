...
#---------------EBST  公共部分 ----------------------
class pub_captcha(unittest.TestCase):

    def setUp(self):

        # 文档获取验证码接口：http://beta-store.elfbar.com/captcha
        # 实际地址：https://beta-store.elfbar.com/coreapi/captcha

        self.Url = Url().test_url()+"/coreapi/captcha"
        print(self.Url)
