...
#---------------EBST  公共部分 ----------------------
class pub_recent_product_view(unittest.TestCase):

    def setUp(self):

        # 文档获近期浏览接口：http://beta-store.elfbar.com/recent-visit-goods?{goods_ids=}

        self.Url = Url().test_url()+"/coreapi/recent-visit-goods"
        print(self.Url)
    ...
    def testcase_001(self):
        ...
        params={"goods_ids":981}
        result = requests.get( self.Url, data=payload,headers=headers,params=params)
        result = result.json ()
        print ( result )
