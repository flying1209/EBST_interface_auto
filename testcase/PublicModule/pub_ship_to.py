...
#---------------EBST  公共部分 ----------------------
class pub_ship_to_list(unittest.TestCase):

    def setUp(self):

        # 文档获取ship to列表接口：http://beta-store.elfbar.com/ship-to-list
        # 实际地址：https://beta-store.elfbar.com/coreapi/ship-to-list

        self.Url = Url().test_url()+"/coreapi/ship-to-list"
        print(self.Url)
