...
#---------------EBST  公共部分 ----------------------
class pub_search_keywords(unittest.TestCase):

    def setUp(self):

        # 文档获搜索关键字模糊查询接口：http://beta-store.elfbar.com/search/so
        # 实际地址：https://beta-store.elfbar.com/coreapi/search/so

        self.Url = Url().test_url()+"/coreapi/search/so"
        print(self.Url)
